...
import { taskFormHelpers as helpers, taskFormRequiredFields } from 'components/Forms/Tasks/helpers';
...
const EditTask = ({ route, navigation }) => {
  ...
  const [res] = useQuery({ query: viewTaskQuery, variables: { id: taskId, pause: isNew }, requestPolicy: 'network-only' });
  const [editRes, executeMutation] = useMutation(upsertTaskMutation);

  const initialValues = React.useMemo(() => setInitialValues(res.data?.task, route?.params?.category, initialValuesSet), [res.data, route.params]);
