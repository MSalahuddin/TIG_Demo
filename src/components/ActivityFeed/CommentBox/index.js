...
const CommentBox = ({ feedId, refreshFeed }) => {
    ...
    return (
        <Box backgroundColor={"#fff"}  justifyContent={'center'} position={"relative"} height={"100%"}  borderTopColor={colors["gray scale/10"]} borderTopWidth={1} width={"100%"}>
            <Input
                multiline
                style={styles.input}
                ...
                icon={
                    (style) => t(isFocused,
                        <Button
                            ...
                            shadow={false}
                            borderRadius={100}
                            icon={(style) => <Icon  {...style} name={'arrow-upward-outline'} pack={"eva"} fill={"#22272F"} />}
                            {...style}
...
const styles = {
    ...
    btn: { borderRadius: 100, height: 18, width: 18, backgroundColor: colors["gray scale/5"], zIndex: 10, borderWidth: 0, },
}
