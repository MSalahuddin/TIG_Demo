...
const Header = ({
  ...
  const renderAction = React.useCallback(
    (
      { icon, pack = 'eva', text, left, status: buttonStatus, badge, ...action },
      i,
    ) => {
      const comp = text ? (
        <Button
        ...
        <TouchableOpacity activeOpacity={0.7} {...action} style={styles.actionIconContainer}>
          <TopNavigationAction
            activeOpacity={0.7}
            icon={s =>
  ...
  const renderLeftActions = React.useCallback(() => {
    const action = actions?.filter(a => a.left && (a?.icon|| a?.text)).slice(0, 1);
    if (action?.length) {
              ...
              {title &&
                (typeof title === 'string' ? (
                  <Text
                    ...
                    appearance={appearance}
                    category="label"
                    style={[componentStyles.title, titleStyle, styles.title]}>
                    {title}
                  </Text>
                ) : (
                  title
